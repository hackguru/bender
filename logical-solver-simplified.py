...
def initialWeightsForSquaring(name,i,k,toAdd,leftOvers):
    if k>2*n-1 or k<0 :
        raise IndexError("range of index can only be from 0 to {}".format(2*n-1))
    
    sumTerms = [] if k%2==1 else [parse_expr("{}{}".format(name, int(k/2)))]
    if k==0:
        return sumTerms
    carryTerms = []
    iIndex = 0
    ...
    while iIndex>jIndex:
        carryTerms.append(parse_expr("{}{}&{}{}".format(name,iIndex,name,jIndex)))
        iIndex -= 1
        jIndex += 1
    toReturn = sumTerms + list(reversed(carryTerms)) + toAdd
    interim = []+toReturn
    count = len(interim)
    while count>1:
        nextInterim=[]
...
for x in range(2*n):
    goesToHigherOrder = []
    print(initialWeightsForSquaring('a',n,x,toAdd,goesToHigherOrder))
    toAdd = goesToHigherOrder
...
#	    rightSideLeftOvers = [sympy.simplify_logic(rightSideTerm.subs(n_Subs)) for rightSideTerm in rightSideLeftOvers]
#	
...
#	if newKkey in D:
#		substitutes[D[newKkey]] = equations[-1][0]^sympy.simplify_logic(D[newKkey]^equations[-1][1])
#	else:
